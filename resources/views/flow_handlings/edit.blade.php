...
@section('title', 'Edit Flow Handling')

@section('content')
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <form action="{{ route('flow_handlings.update', $flowHandling->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="{{ $field }}" id="{{ $field }}" class="w-full border-gray-300 rounded dark:bg-gray-700 dark:text-white dark:border-gray-600 @error($field) border-red-500 @enderror">
                            ...
                            @foreach ($options as $option)
                                <option value="{{ $option }}" {{ old($field, $flowHandling->$field) == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                ...
                @foreach ([
                    ...
                    <div>
                        ...
                        <input type="{{ $type }}" name="{{ $field }}" id="{{ $field }}"
                               ...
                               value="{{ old($field, $flowHandling->$field) }}">
                        @error($field) <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                @endforeach
            </div>
            ...
            <div class="flex justify-start gap-3">
                ...
                <a href="{{ route('flow_handlings.index') }}" class="px-4 py-2 text-white bg-gray-500 rounded hover:bg-gray-600">Kembali</a>
            </div>
        </form>
    </div>
@endsection
